...
async def parse_ozon_page_async(page, timeout=15000) -> dict:
    ...
    # АНТИБОТ / КАПЧА
    #
    # Ozon иногда вместо реальной карточки отдаёт HTTP 200 с
    # заглушкой-испытанием (антибот/капча) - page.goto() формально
    # УСПЕШЕН, поэтому _goto_with_retry() её не ловит, а
    # wait_for_selector() выше просто уходит в таймаут (нужных
    # виджетов на такой странице нет) и код молча продолжал парсить
    # ЭТУ страницу как обычную: <title> капчи (буквально "Antibot
    # Captcha") подставлялся как card.title и уходил в классификацию,
    # где иногда даже находился случайный "кандидат" с confidence,
    # которого хватало, чтобы не попасть под ручную проверку - такой
    # результат оседал в runtime_cards.json и на СЛЕДУЮЩЕМ запуске
    # того же файла подставлялся уже как будто надёжный (см.
    # разбор в products-dict-gradation-audit.md).
    #
    # Проверяем по сырому HTML ДО парсинга - тот же принцип
    # обнаружения, что уже используется в более старых/тестовых
    # скриптах (processors/ozon_excel_processor.py,
    # parser/ozon_fast_parser.py), но здесь это боевой путь. По
    # прямому указанию Яна: в этом случае наименование/код НЕ
    # трогаем вообще (см. processors/ozon_auto_processor.py -
    # проверка card.antibot сразу после парсинга).
    # ======================================================
    ...
    if antibot:
        log.warning(
            "OZON ANTIBOT/CAPTCHA обнаружен: url=%s",
            page.url,
        )
        # composer_task (см. выше) - отдельный сетевой запрос,
        # запущенный ПАРАЛЛЕЛЬНО с ожиданием DOM, ещё может быть в
        # работе. Раньше он всегда дожидался ниже (await composer_task)
        # - при раннем return здесь его никто не ждёт, и без явной
        # отмены задача осталась бы "болтаться" в event loop поверх
        # уже переиспользуемой для следующего товара вкладки.
