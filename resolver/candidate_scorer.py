...
class CandidateScorer:
    ...
    def _field_score(self, candidate, text, phrase, weight, source):
        ...
        text = normalize_dictionary_name(text).lower().strip()
        phrase = normalize_dictionary_name(phrase).lower().strip()
        ...
        if text == phrase:
            ...
            return
        # ======================================================
        # ТОЧНОЕ СЛОВОСОЧЕТАНИЕ ВНУТРИ ТЕКСТА
        # ======================================================
        pattern = (
                r"(?<!\w)"
                + re.escape(phrase)
        ...
        if re.search(pattern, text):
            ...
            # а внутри текста встречается полностью,
            # даём почти полный вес.
            # --------------------------------------------------
            if len(words_phrase) > 1:
                candidate.add(
                    source,
                    int(weight * 0.95),
                    phrase,
            ...
            # Одно слово внутри более длинного названия.

            candidate.add(source, int(weight * 0.45), phrase,)
