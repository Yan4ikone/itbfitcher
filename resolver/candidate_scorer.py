    ...
    def score(self, candidate, parsed, specs_weight=150):
        ...
        if required_context:
            ...
            if not any(word in haystack for word in required_context):
                return candidate
        # ------------------------------------------------------------
        # ДОБАВЛЕНО (обновление 19): "excludes_context" - зеркальная
        # противоположность requires_context выше, для случая, когда
        # ОБЩЕЕ слово нужно ИСКЛЮЧИТЬ при наличии более специфичного
        # уточнения рядом (пример - "чипсы": обычные картофельные
        # чипсы и ПРОТЕИНОВЫЕ чипсы (это пищевая добавка/БАД, у неё
        # свой отдельный код) делят слово "чипсы", и одного алиаса
        # "протеиновые чипсы" на товаре 'бад' недостаточно - простое
        # прямое совпадение слова "чипсы" в CLEANED+TITLE (600) всё
        # равно набирает больше очков, чем совпадение по алиасу
        # (TITLE_ALIAS+CLEANED_ALIAS=550), и общий кандидат "чипсы"
        # побеждал. Если у товара задан этот список слов - кандидат
        # НЕ участвует в подборе кода, когда рядом в тексте карточки
        # ЕСТЬ хотя бы одно из этих слов (то есть это, скорее всего,
        # другой, более специфичный товар/значение).
        # ------------------------------------------------------------
        excluded_context = info.get("excludes_context")

        if excluded_context:

            haystack = parsed.get("search_text", "") or ""

            if any(word in haystack for word in excluded_context):
                return candidate
