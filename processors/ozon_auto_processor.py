...
class OzonAutoProcessor:
    ...
    # ==========================================================
    # Комментарии ниже по коду ("4 ПОСТОЯННЫЕ ВКЛАДКИ", "без неё 4
    # вкладки бьют по Ozon без остановки") описывают исходный дизайн
    # на 4 воркера - но константа стояла на 2, из-за чего реальный
    # прогон (Test_machine_2, 2026-09-18) давал ~6с на товар при
    # среднем времени загрузки страницы в разы больше паузы между
    # запросами: 2 воркера означают, что половина процессорного
    # времени вкладки простаивает, ожидая своей очереди. Возвращено
    # к 4, как и было задумано изначально по коду ниже - задержка
    # WORKER_DELAY_MIN/MAX уже ограничивает частоту запросов ОТ
    # ОДНОГО воркера независимо от числа воркеров, поэтому 4 воркера
    # с той же паузой должны примерно вдвое сократить среднее время
    # на товар. Стоит проверить на реальном прогоне, не участится ли
    # антибот-блокировка Ozon при 4 параллельных вкладках - если да,
    # это единственный параметр, который стоит откатить обратно.
    MAX_WORKERS = 4
