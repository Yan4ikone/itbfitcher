...
class OzonAutoProcessor:
    ...
    def apply_result(self, ws, row, card, result):
        ...
        if result.review and alternatives:
            ...
            # входит в режим редактирования - выбрать вариант, начав
            # печатать код, было невозможно, а помнить/не ошибиться в
            # 10 цифрах неудобно (жалоба Яна). Список - те же коды, что
            # и в комментарии; комментарий поясняет, что означает
            # каждый код, список позволяет выбрать готовый код кликом.
            # См. utils/dropdown_helpers.py::build_alternatives_validation,
            # тот же приём уже используется в
            # modules/dropdown_manager.py::apply_specific_dropdowns.
        ...
        # спорности только из консольного лога). Подсвечиваем сразу в
        # самой таблице - не только когда C{row} пустая, но и когда код
        # проставлен, просто неуверенно.
        #
        # ВАЖНО: красим/курсивим ТОЛЬКО ячейку кода (C). Раньше здесь
        # так же красилась и B (наименование) - шрифт/заливка описания
        # товара к спорности КОДА отношения не имеют и не должны
        # меняться. По просьбе Яна - наименование не трогаем никогда
        # (ни заливкой, ни шрифтом/курсивом), только код.
        # ------------------------------------------------------
        if result.review:
            cell = ws[f"C{row}"]
    ...
    def run(self):
        ...
        try:
            ...
            # ДОБАВЛЕНО: настоящий выпадающий список (Data Validation)
            # с материальными/иными вариантами известного товара - для
            # тех случаев, когда программа выбрала вариант неверно, или
            # материал не указан в тексте карточки, но виден по фото
            # (жалоба Яна). Раньше эта функция вызывалась только в
            # ручном режиме (excel/processor.py) - в Ozon-авто
            # конвейере её не было вообще, куратор мог поправить код
            # только вручную набрав 10 цифр. Строки, для которых уже
            # построен комментарий с alternatives (см. apply_result
            # выше), пропускаются - см. защиту в
            # modules/dropdown_manager.py::apply_specific_dropdowns.
            self.log("Выпадающие списки...")

            apply_specific_dropdowns(
                ws,
                desc_col_idx=2,        # колонка B
                code_col_idx=3,        # колонка C
                max_row=postprocess_last_row,
            )
