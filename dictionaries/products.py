PRODUCTS = {
    ...
    # обновление 15) - разбор жалобы Яна: карточка "10 черных резиновых
    # заглушек для микрофонного разъема PC DVD 6,35 мм" не находила ни
    # одного кандидата товара в словаре вообще (такого товара просто не
    # было) и в итоге получала код "наушники" ЕДИНСТВЕННО через слабое
    # совпадение по breadcrumb-категории маркетплейса (Ozon отнёс товар к
    # категории наушников/гарнитур, т.к. это аксессуар для микрофонного
    # разъёма) - "глупая ошибка", как её описал Ян, хотя и в
    # заголовке, и в описании товар назван прямо. Код 4016999708 -
    # тот же катч-олл код для прочих резиновых изделий (товарная позиция
    # 4016), что уже используется в этом словаре для аналогичных мелких
    # резиновых предметов (резинка, патрубок, фоамиран, шайба
    # подшипника) - выбран по прямому прецеденту.
    'заглушка': {
        'code': '4016999708',
        'patterns': [
            'заглушк.*',
    ...
    # ДОБАВЛЕНО (2026-09-26, products-dict-gradation-audit.md,
    # обновление 15) - разбор жалобы Яна: карточка "Запчасти
    # автомобильные, арт. I194080A4" (заголовок совершенно общий, без
    # единого полезного слова) вообще не находила ни одного кандидата -
    # ни один товар в словаре не совпадал, хотя, по словам Яна, в
    # описании чётко сказано "ручка из пластика". В словаре при этом
    # был только один товар "ручка" - "ручка мебельная" (сразу ниже),
    # никакого автомобильного/дверного варианта не было вообще. НЕ
    # заводим бытовую "ручка" (без уточнения) как отдельный товар - это
    # частое бытовое слово (в т.ч. "ручка" = пишущий инструмент), голое
    # совпадение с ним было бы слишком широким и рискованным. Вместо
    # этого - отдельный товар "ручка автомобильная" с requires_context,
    # чтобы он вообще участвовал в подборе кода, только когда рядом
    # действительно есть автомобильный/дверной/кузовной контекст (см.
    # requires_context в resolver/candidate_scorer.py - кандидат
    # пропускается целиком, если ни одного из этих слов нет в тексте
    # карточки). Код 8708299009 (товарная позиция 8708.29 - "прочие
    # части и принадлежности кузовов") - тот же код, что уже
    # используется в этом словаре для похожего случая "подножка"
    # (тоже общая, некрупная деталь кузова без более узкой подпозиции) -
    # выбран по прямому прецеденту, не наугад. Требует подтверждения
    # Яна, если у него есть более точный код для конкретно дверных
    # ручек.
    'ручка автомобильная': {
        'code': '8708299009',
        'patterns': [
            'ручк.*для.*двер.*автомоб',
            'автомоб.*ручк',
        ],
        'aliases': [
    ...
    'чашка': {
        ...
        'dropdown': {
            ...
            'variants': [
                ...
                # ДОБАВЛЕНО (2026-09-26, products-dict-gradation-audit.md,
                # обновление 15) - разбор жалобы Яна: карточка "Чашка 1
                # шт" с описанием "Металлическая десертная чашка"
                # корректно определяла material='металл', но ни один из
                # трёх вариантов выше не металлический - резолвер не мог
                # найти совпадение ни по оси, ни по фоллбеку material, и
                # уходил в DROPDOWN_FIRST (первый вариант = керамика),
                # хотя выбирать было ЕСТЬ из чего - просто нужного
                # варианта не было в списке. Код 7323990000 (посуда/
                # утварь из чёрных металлов, товарная позиция 7323) -
                # тот же код, что уже многократно используется в этом
                # словаре для металлической посуды/кухонной утвари
                # (доска, казан, масленка, гастроёмкость, мантоварка и
                # т.д.) - выбран по прямому прецеденту, не наугад.
                {
                    'code': '7323990000',
                    'name': 'Металл',
                    'group': 'metal',
