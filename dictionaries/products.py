PRODUCTS = {
    ...
    'ботинки': {
        # ДОБАВЛЕНО: по решению Яна ("обувь мы различаем только кожа и
        # пол/возраст") - оба прежних варианта различались только
        # материалом верха (резина/пластик vs текстиль), без кожи и
        # без пола/возраста - это НЕ та ось, которую нужно сохранять
        # (products-dict-gradation-audit.md, обновление (8)). Свели к
        # одному, более общему коду (резина/пластик верх, закрывает
        # лодыжку).
        'code': '6402919000',
        'patterns': [
            'мотооб',
    ...
    'вкладыш для обуви': {
        # ДОБАВЛЕНО: варианты - "вкладные стельки" vs общая "прочие" -
        # не материал/пол, просто специфичный код vs общий каталожный
        # хвост той же товарной позиции. Свели к более точному
        # (вкладные стельки и другие сменные детали - это и есть
        # "вкладыш для обуви").
        'code': '6406905000',
        'patterns': [],
        'aliases': [],
    ...
    'галоши': {
        # ДОБАВЛЕНО: по решению Яна - варианты различались материалом
        # верха (резина vs пластик), без кожи/пола-возраста - свели к
        # одному коду (водонепроницаемая обувь с верхом из резины -
        # это и есть классические галоши).
        'code': '6401921000',
        'patterns': [],
        'aliases': [
            'galoshi',
    ...
    'диван раскладной': {
        # ДОБАВЛЕНО: по решению Яна ("мебель - по материалу") - коды
        # официально различаются именно материалом ("мебель
        # металлическая прочая" vs мягкая мебель для сидения,
        # трансформируемая в кровати) - оставили как материальную ось,
        # только нормализовали group под конвенцию MATERIAL_GROUP_EN
        # (было 'other' у обоих - ничего не резолвилось).
        'code': '',
        'patterns': [
            'диван.*раскл',
        ],
    ...
    'пинетки': {
        # ДОБАВЛЕНО: варианты различались только материалом подошвы
        # (прочая vs резина/пластик), верх у обоих текстильный - не
        # кожа/пол-возраст - свели к одному коду.
        'code': '6405209900',
        'patterns': [],
        'aliases': [],
    },
    ...
    'шуруповерт': {
        # ДОБАВЛЕНО: официально коды различаются именно "способен
        # работать без внешнего источника питания" (=аккумуляторный)
        # vs обычный/сетевой - ровно ось Яна электро/аккумуляторный.
        # Требует нового ключа 'battery' в CHARACTERISTIC_ALIASES
        # (dictionaries/all_dictionaries.py) - добавлен.
        'code': '',
        'patterns': [
            'шурупове.*',
        ],
    ...
    'электропила': {
        # ДОБАВЛЕНО: варианты различались типом пилы (дисковая/цепная/
        # прочая), не ручной/бытовой/электро/аккумуляторный - свели к
        # общему коду ("пилы: прочие"). Отдельные точные товары
        # ('электропила дисковая', код 8467223000) остаются как были.
        'code': '8467229000',
        'patterns': [],
        'aliases': [],
    },
