...
class DecisionEngine:
    ...
    def decide(self, card, remember=True):
        ...
        # обновление 17) - разбор кейса Яна "валик" ("в описании
        # Роликовая щетка 1 * Фильтр... почему не подключился ИИ").
        #
        # До этой правки условие ниже было ОДНИМ большим `if`
        # (should_try_image and self.image_processor) - если карточка
        # была спорной, но self.image_processor оказался None
        # (сорвалась инициализация ИИ-движка в __init__, например нет
        # ключа нужного сервиса - см. try/except там), шаг 7.5 просто
        # молча пропускался целиком, БЕЗ единой строчки где-либо. То
        # же самое было и дальше внутри блока: если process() отработал
        # без исключения, но card.image_description осталась пустой
        # (нет фото у товара, картинка не загрузилась, ИИ вернул пустой
        # ответ), или если ИИ дал описание, но retried-результат не
        # прошёл проверку на замену (см. комментарий ниже про
        # has_direct_text_support) - ни то, ни другое нигде не
        # фиксировалось. В сумме куратор, глядя на карточку "под
        # вопросом", не мог понять ни пробовал ли ИИ вообще
        # подключиться, ни что он нашёл, ни почему это не помогло -
        # приходилось гадать. Теперь каждая из этих веток пишет причину
        # в result.trace - а он с этого же обновления полностью
        # попадает в файл logs/YYYY-MM-DD.log на каждую карточку (см.
        # modules/decision_logger.py), а не теряется молча.
        # ==========================================================
        if should_try_image and not self.image_processor:
            result.trace.add(
                "IMAGE_UNAVAILABLE",
                "ИИ-распознавание по картинке должно было запуститься "
                "(код не найден / нет прямого текстового подтверждения / "
                "карточка под вопросом), но self.image_processor не "
                "инициализирован - см. 'IMAGE PROCESSOR INIT ERROR' в "
                "errors_YYYY-MM-DD.log за дату старта программы - шаг "
                "пропущен"
            )

        if should_try_image and self.image_processor:
            ...
            if getattr(card, "image_description", ""):
                ...
                if retried.code and (
                    ...
                    # ДОБАВЛЕНО: ИИ дал описание, но оно не изменило
                    # решение - либо повторная классификация вообще не
                    # дала кода (retried.code пуст, т.е. описание с
                    # картинки не сняло исходную неоднозначность), либо
                    # код уже был и retried не дал прямого текстового
                    # подтверждения (see comment above) - остаётся
                    # прежний result.
                    result.trace.add(
                        "IMAGE_FALLBACK_REJECTED",
                        f"ИИ вернул описание ({card.image_description}), "
                        f"но результат НЕ заменён: retried.code="
                        f"{retried.code or '-'!r}, "
                        f"retried.has_direct_text_support="
                        f"{retried.has_direct_text_support}, "
                        f"had_code_before_image={had_code_before_image}"
                    )
            else:
                # ДОБАВЛЕНО: process() отработал без исключения, но
                # card.image_description осталась пустой - нет фото
                # товара, картинка не загрузилась, или ИИ-движок вернул
                # пустой ответ (подробности при наличии исключения - в
                # errors_YYYY-MM-DD.log, см. также
                # processors/card_image_processor.py).
                result.trace.add(
                    "IMAGE_FALLBACK_EMPTY",
                    "ИИ-распознавание по картинке запущено, но описание "
                    "не получено (нет фото товара, картинка не "
