...
class Morphology:
    ...
    @lru_cache(maxsize=100000)
    def case(self, word: str) -> str:
        """Падеж слова ('nomn', 'gent', 'datv', ...) или "" если у
        части речи падежа нет (глагол, наречие) либо разбор не
        удался. Нужен для различения "сиденье" (сам товар,
        именительный падеж) от "...спинки автомобильного СИДЕНЬЯ"
        (родительный падеж - деталь ДЛЯ/У сиденья, а не само
        сиденье) - см. resolver/candidate_scorer.py::
        _weaken_if_modifier_context_lemma."""

        if not word:
            return ""

        tag = self.morph.parse(word)[0].tag.case
