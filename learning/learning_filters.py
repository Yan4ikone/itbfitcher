...
# ------------------------------------------------------------------
# "ЖЁСТКИЙ" мусор - ЛЮБОЕ вхождение где-либо во фразе бракует весь
# алиас целиком, а не только когда алиас ПОЛНОСТЬЮ из него состоит.
#
# _STRONG_TRASH_MARKERS выше появился именно из-за этого пробела:
# "футболка акции скидки" не ловился общей проверкой "все слова
# мусорные" (ловится только если КАЖДОЕ слово в _TRASH_WORDS - а
# "футболка" им не является), поэтому пришлось руками завести
# отдельный маленький список маркеров с более строгой проверкой.
#
# Тот же пробел актуален и для слов, которые куратор САМ добавляет в
# TRASH_MARKETING через кнопку "В мусор" в окне обучения
# (learning/dictionary_editor.py::add_trash_word) - IGNORED_ALIAS_WORDS
# специально остаётся "мягким" (там служебные слова вроде "для"/
# "мужской", нормальные ВНУТРИ валидного алиаса вроде "рубашка
# мужская"), а вот TRASH_MARKETING/TRASH_MARKETPLACE/TRASH_PACKAGE -
# это как раз то, что куратор явно пометил как "никогда не предлагай
# это снова". Раньше такое слово реально блокировало алиас, только
# если он был ИЗ НЕГО ОДНОГО или совпадал целиком - стоило ему попасть
# в один ряд с нормальным словом ("сумка premium класса") - и алиас
# снова предлагался, несмотря на то что "класса" уже был в мусоре.
#
# Проверяем как отдельные слова, так и МНОГОСЛОВНЫЕ мусорные фразы
# (TRASH_MARKETPLACE/TRASH_PACKAGE хранят и такие - "для дома",
# "подарочный набор") - через границы слов, чтобы не ловить "уп" в
# середине "суп"/"группа".
_HARD_TRASH_PHRASES = tuple(
    sorted(
        (
            phrase.strip().lower()
            for phrase in (TRASH_MARKETING | TRASH_MARKETPLACE | TRASH_PACKAGE)
...
_HARD_TRASH_PATTERN = (
    re.compile(
        r"(?<!\w)(?:"
        + "|".join(re.escape(phrase) for phrase in _HARD_TRASH_PHRASES)
        + r")(?!\w)"
    )
    if _HARD_TRASH_PHRASES
    else None
)
...
def is_valid_alias(alias, product_name=""):
    ...
    # То же самое, но для ВСЕГО, что куратор/разработчик уже занёс в
    # TRASH_MARKETING/TRASH_MARKETPLACE/TRASH_PACKAGE - не только для
    # маленького ручного списка _STRONG_TRASH_MARKERS выше (см.
    # комментарий у _HARD_TRASH_PATTERN).
    if _HARD_TRASH_PATTERN and _HARD_TRASH_PATTERN.search(alias):
        return False
