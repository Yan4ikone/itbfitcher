...
# ------------------------------------------------------------------
# ТРАНСЛИТЕРАЦИЯ (кириллица -> латиница), схема как у слагов Ozon в
# URL карточек ("щетка для уборки" -> "shchetka dlya uborki",
# "разъем" -> "razem", "держатель" -> "derzhatel" - твёрдый/мягкий
# знак просто выпадают, без апострофа). Нужна для проверки коллизии
# алиаса с существующим названием товара (см. is_valid_alias() ниже,
# reserved_names) - когда алиас случайно взят из slug/URL и попадает
# в словарь ЛАТИНИЦЕЙ, а название конфликтующего товара - кириллицей.
# Направление только кириллица -> латиница: обратное (угадать
# кириллицу по латинице) неоднозначно, а вот кириллица -> латиница по
# этой же схеме, что использует сам Ozon, - детерминировано.
# ------------------------------------------------------------------
_TRANSLIT_MAP = {
...
def transliterate_ru(text):
    ...
    return "".join(
        _TRANSLIT_MAP.get(char, char)
        for char in text
    )


def is_valid_alias(alias, product_name="", reserved_names=None):
    ...
    # ------------------------------------------------------
    # КОЛЛИЗИЯ С ЧУЖИМ ТОВАРОМ
    #
    # Если alias дословно (или в латинской транслитерации slug'а
    # Ozon) совпадает с названием ДРУГОГО, уже существующего в
    # словаре товара - его нельзя добавлять сюда алиасом. Пример из
    # реальной жалобы Яна: "держатель" существует как отдельный товар
    # со своим кодом; поэтому ни "держатель", ни "derzhatel" не могут
    # стать алиасом "держатель для телефона" - иначе любая карточка,
    # где слово "держатель" всего лишь упомянуто мимоходом (магнитное
    # крепление, часы с держателем для ручек), начинает матчиться на
    # чужой товар только из-за общего слова в алиасах.
    #
    # reserved_names - НЕ множество запрещённых форм alias'а (оно и
    # так одно - сам alias), а множество ИМЁН ТОВАРОВ-ВЛАДЕЛЬЦЕВ,
    # которым уже принадлежит эта форма (прямая или транслитерация)
    # как СОБСТВЕННОЕ название - без текущего продукта (вызывающая
    # сторона, см. learning/analyzer.py::_build_reserved_alias_map,
    # уже вычла его). Любой непустой набор здесь = коллизия.
    # ------------------------------------------------------
    if reserved_names:
        return False
