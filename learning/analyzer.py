...
class LearningAnalyzer:
    ...
    def analyze(self):
        ...
        for card in self.runtime.all_cards():
            ...
            # ALIASES
            #
            # ОТКЛЮЧЕНО (2026-09-25, products-dict-gradation-audit.md,
            # обновление 14) - по прямому указанию Яна: "мы не берём
            # мусор из карточки товара, мы предлагаем лишь то, что
            # попадает в ячейку с описанием, если совпадает код...
            # иначе смысла я не вижу к слову крепление добавлять алиас
            # крепления для ручки штатива". _analyze_aliases() (см.
            # ниже по файлу) брала кандидатов НЕ из description
            # (наименование - тот же текст, что видел и подтвердил
            # куратор), а из сырых card.title/card.slug через
            # AliasBuilder.build() (cleaner/alias_builder.py) - причём
            # build() сама явно выбрасывает description из результата
            # (aliases.discard(description)), то есть по конструкции
            # предлагала ТОЛЬКО формулировки, отличные от того, что
            # куратор реально подтвердил. Ровно это и есть источник
            # "мусора" из примера Яна ("крепления для ручки штатива"
            # как алиас общего "крепление"). Единственный алиас,
            # который куратор фактически подтверждает своей правкой -
            # сам description, когда он совпал с существующим товаром
            # по коду - это уже полностью покрыто выше, в
            # _resolve_product() (self._add_alias(report, product_name,
            # product_info, description), строки ~261/271) - тот же
            # текст, что и наименование, и только при совпадении кода,
            # ровно по правилу Яна. Метод _analyze_aliases() и
            # AliasBuilder оставлены в файле неудалёнными (на случай,
            # если понадобится вернуться к более широкому предложению
            # алиасов), но здесь больше не вызываются.
            # --------------------------------------------------
            # --------------------------------------------------
            # DROPDOWN
            # --------------------------------------------------
    ...
    # ALIASES ANALYSIS
    #
    # НЕ ВЫЗЫВАЕТСЯ с 2026-09-25 (обновление 14) - см. подробный
    # комментарий у места вызова в analyze() (было между MATERIAL и
    # DROPDOWN). Предлагает алиасы из card.title/card.slug, а не из
    # подтверждённого куратором description - это противоречит
    # прямому указанию Яна. Оставлено в файле как есть, не вызывается.
    # ==========================================================
    ...
    def _observe_code(self, product_name, product_info, code, card, description):
        ...
        ВАЖНО - ИСТОРИЯ ПРАВКИ (единичная ручная правка терялась):
        раньше счётчик заполнялся ТОЛЬКО кодами, реально увиденными в
        этом прогоне analyze(). Если куратор за сессию поправил ровно
        ОДНУ карточку товара на новый код - в счётчике оказывался
        ровно один РАЗЛИЧНЫЙ код, порог MIN_DROPDOWN_DISTINCT_CODES=2
        не набирался, товар не предлагался как dropdown-кандидат, а
        сама карточка тут же помечалась обработанной (см. analyze()) -
        то есть повторно к ней уже не возвращались, и подтверждённая
        человеком правка исчезала без следа: при следующей обработке
        того же наименования резолвер снова ставил старый неверный
        плоский code.

        Теперь при ПЕРВОМ обращении к товару в этом прогоне счётчик
        засевается его текущим плоским code из products.py - это уже
        известный, реально стоящий в словаре код, а не гипотеза. Если
        куратор наблюдался всего с ОДНИМ новым (другим) кодом - вместе
        с засеянным старым это уже 2 РАЗНЫХ кода, и порог расхождения
        срабатывает сразу же, за одну правку - в духе того же принципа,
        что и MIN_DROPDOWN_CODE_OCCURRENCES=1 (правка куратора - это
        подтверждённый факт, а не гипотеза, которую нужно накапливать).
        Если же куратор просто подтвердил тот же самый код (никакого
        расхождения на самом деле нет) - засеянный и увиденный код
        совпадают, distinct остаётся равным 1, и, как и раньше, ничего
        не предлагается - ложных срабатываний это не добавляет.
        """
        ...
        counter = self._code_observations.get(product_name)

        if counter is None:
            counter = Counter()
            self._code_observations[product_name] = counter

            default_code = str(product_info.get("code", "")).strip()

            if (
                default_code
                and default_code != "0"
                and default_code not in known_material_codes
            ):
