...
def comment_box_size(text):
    """Размер (width, height в точках) для openpyxl.comments.Comment
    под ФАКТИЧЕСКОЕ содержимое - используется вместе со списком
    альтернатив (см. processors/ozon_auto_processor.py::apply_result,
    engines/result_engine.py::set_comment - оба вешают комментарий со
    списком "код — название" на ячейку кода при result.review).

    ДОБАВЛЕНО (2026-09-25, products-dict-gradation-audit.md,
    обновление 14) - жалоба Яна: "с выпадающим списком отдельная
    беда, я вижу только описание и 1й код из списка, на этом окно
    заканчивается, а выбирать приходится из 4 или иногда даже 5, это
    жутко неудобно". Причина - Comment(text, author) без явных
    width/height берёт дефолты openpyxl (144x79 точек, это ~5x2 см) -
    туда реально помещается только заголовок "Выбрать код вручную:" и
    начало первой строки, остальные 3-4 альтернативы обрезаются
    ПОЛНОСТЬЮ, даже без прокрутки. Считаем размер по самой длинной
    строке (ширина) и количеству строк (высота), с минимумом/
    максимумом - чтобы короткий список (1-2 альтернативы) не занимал
    пол-экрана, а длинный (после единичных случаев с десятком
    вариантов) не улетал за разумные пределы.
    """
    ...
    # Нижняя граница высоты - те же 79pt, что и дефолт самого
    # openpyxl.comments.Comment: при малом числе строк (1-3) формула
    # ниже даёт МЕНЬШЕ дефолта (напр. 3*14+20=62pt) - а результат не
    # должен быть хуже того, что было раньше ни в одном случае.
    width = max(180, min(420, max_line_len * 5.5 + 20))
    height = max(79, min(260, line_count * 14 + 20))
